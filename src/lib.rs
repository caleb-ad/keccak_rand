...
impl Keccak<'_>{
    ...
    fn column_parity<'a>(&'a self, x: usize, z: usize) -> Bit{
        return self.get(x,0,z) ^ self.get(x,1,z) ^ self.get(x,2,z)
            ^ self.get(x,3,z) ^ self.get(x,4,z);
    }
        ...
        for x in 0..(5 as i64){
            for z in 0..(self.w as i64){
                let d = self.column_parity(((x-1) % 5).abs() as usize, z as usize) ^
                    self.column_parity((x+1) as usize % 5, ((z-1) % self.w as i64).abs() as usize);
                for y in 0..(5 as usize){
                    self.set(x as usize, y, z as usize, self.get(x as usize, y, z as usize) ^ d);
                }
            }
        }
    ...
    fn rho<'a>(&'a mut self){
        for z in 0..self.w { self.set(0, 0, z, self.get(0,0,z)) }
    ...
    fn pi<'a>(&'a mut self){
        for x in 0..(5 as usize){
            for y in 0..(5 as usize){
                for z in 0..self.w{
                    self.state1.set(self.get_idx(x, y, z), self.get((x + 3*y) % 5, x, z));
                }
            }
        }
        mut_ref_swap!(self.state, self.state1, VLA<Bit>);
    ...
    fn chi<'a>(&'a mut self){
        for x in 0..(5 as usize){
            for y in 0..(5 as usize){
                for z in 0..self.w{
                    self.set(x, y, z, self.get(x,y,z) ^ ((self.get((x+1) % 5, y, z) ^ 1) & self.get((x+2) % 5, y, z)));
                }
            }
        }
        mut_ref_swap!(self.state, self.state1, VLA<Bit>);
            ...
            for x in 0..5 as usize{
                for y in 0..5 as usize{
                    for z in 0..self.depth(){
                        self.set_state(x, y, z, self.get(x,y,z) ^ message.get(self.depth() * (5*y + x) + z));
                    }
                }
            }
